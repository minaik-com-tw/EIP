...
9912	偉聯	AIC";
        ...
        if (Cache[CACHE_KEY] == null)
        {
            ...
            rs.Open("select  name,email,empid  from dguser  ");
            //'" + Request.QueryString["text"] + "%' "
            while (!rs.EOF)
            {
                t.Rows.Add(rs["email"].ToString(), rs["name"].ToString(), rs["name"].ToString(), rs["name"].ToString());
                rs.MoveNext();
            }
            rs.Close();
           ...
           // string sql = "  select   ima01,ima02,ima63     from minaik.ima_file   ";

           // //string sql = "  select a.bmb01 bmb01,b.ima02 ima02,b.ima021 ima021 from minaik.bmb_file a left join minaik.ima_file b on a.bmb01=b.ima01 group by a.bmb01,b.ima02,b.ima021   ";
