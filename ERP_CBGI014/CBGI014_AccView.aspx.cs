...
namespace ERP_CBGI014
{
    public partial class CBGI014View : SmoothEnterprise.Web.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
              //  Response.Write(" select revieweruid,text  from  dgflowqueue   where requesturl like '%" + Request.QueryString[0] + "%'" +
              ...
              // "/erp_cbgi014/cbgi014edit.aspx')");
               ...
               //Request.RawUrl.ToString().ToLower().Replace("_accview", "edit") + "')");
                ...
                //    "'  AND reviewdate IS NULL AND qseq is not null  and  text='會計1' and  revieweruid='"+this.CurrentUser.ID+"'");
                //if (!rs.EOF)
                //{
        ...
        protected void Page_PreRender(object sender, System.EventArgs e)
        {
            ...
            if (_NotifyReview)
            {
                ...
                while (!rss.EOF)
                {
                    ...
                    string purl = "http://eip.minaik.com.tw/ERP_CBGI014/CBGI014_AccView.aspx?rowid=" + Request.QueryString[0]; 
                        //+ rss["requesturl"].ToString().ToLower().Replace("edit", "View");
        ...
        private void gvBind(GridView gv)
        {
            ...
            string str = "SELECT     tc_cbb05, aag02,tc_cbb22, tc_cbb08,"+
                                "    tc_cbb03,fac02,tc_cbb18,tc_cbb091,tc_cbb10," +
                                "            tc_cbb13f,tc_cbb11,tc_cbb12, tc_cbb14f,"+
                                 "           tc_cbb14, tc_cbb16,tc_cbb21,azf03, tc_cbb17,"+
                                 "    convert(char(10),faj26,112)      faj26,round(faj31,2) faj31,fajowner   FROM [EIPB].[dbo].[CBGI014_Body] where  rowid='" + Request.QueryString["rowid"] + "'   order by  tc_cbb03 asc";
        ...
        {
            SqlConnection conn = new SqlConnection(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["EIPAConnectionString"].ConnectionString);
            string str = "SELECT  *   FROM [EIPB].[dbo].[CBGI014_Body] where  rowid='" + Request.QueryString["rowid"] + "'";
        ...
        {
            double sum = 0;
            //Response.Write(_gd.Rows.Count + "<br>");
            if (_gd.Rows.Count > 0)
            {
                //Response.Write(_gd.Rows[0].Cells.Count + "<br>");

                for (int i = 2; i < _gd.Rows[0].Cells.Count; i++)
                {
                    sum = 0;
                    if ((i.ToString() == "8")||(i.ToString() == "13"))
                    {
                        for (int j = 0; j < _gd.Rows.Count; j++)
                        {
                           // Response.Write(_gd.Rows[j].Cells[i].Text + "<br>");
        ...
        protected void FlowFeedback1_FlowStop(object sender, SmoothEnterprise.Flowwork.UI.WebControl.FlowStopEventArgs e)
        {
            ...
            if (e.ResultType == SmoothEnterprise.Flowwork.Control.ReviewResultType.Complete)
            {
                ...
                ds.Open("select top 1 email,name  from dgflowqueue a " +
                    ...
                    "  requesturl='" + Request.RawUrl.ToString().Trim().ToLower().Replace("_accview", "edit") + "'   order by a.initdate asc    ");
                if (!ds.EOF)
                {
            ...
            {
                Sql = "update [EIPB].[dbo].[CBGI014_Head] set   status='R' where  rowid='" + Request.QueryString["rowid"] + "'";
                ...
                if (!ds.EOF)
                {
                    ...
                    requesturl = "http://eip.minaik.com.tw" + Request.RawUrl.ToString().Trim().ToLower().Replace("_accview", "edit");
