...
public partial class cimi100_base : SmoothEnterprise.Web.Page
{
	...
	protected void Page_Load(object sender, System.EventArgs e)
	{
        ...
        if (!this.IsPostBack)
        {
            ...
            rs.Open("SELECT   bass_no,name FROM " + db1 + "cimi100_base where class_no='plantid' order by order_no");
            while (!rs.EOF)
            {
	...
	protected void DataList1_OnRenderCell(object sender, System.Data.DataRowView rs, SmoothEnterprise.Web.UI.WebControl.DataColumn column, SmoothEnterprise.Web.UI.WebControl.DataCell cell, System.EventArgs e)
	{
		...
		{   
 	 	 	...
 	 	 	{
 	 	 	 	cell.LinkURL = "cimi100_baseEdit.aspx?id="+rs["id"].ToString();
 	 	 	}
	...
	private void Query()
	{
        ...
        if (this.FIELD_class_no.Text != "")
 	 	{
 	 	 	if (whereis!="")
 	 	 	 	whereis = whereis + " AND";
 	 	 	whereis = whereis + " a.class_no = '" + this.FIELD_class_no.Text + "'";
 	 	}
        ...
        DataList1.SQL = "SELECT a.*,b.name bname,c.name cname,d.name dname FROM " + db1 + "cimi100_base a left join " + db1 + "cimi100_class b on a.class_no=b.class_no " +
