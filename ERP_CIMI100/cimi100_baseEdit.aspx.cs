...
public partial class cimi100_baseEdit : SmoothEnterprise.Web.Page
{
	...
	protected void Page_Load(object sender, System.EventArgs e)
	{
		...
		try
		{
			if (!this.IsPostBack)
			{
                ...
                rs2.Open("SELECT * FROM " + db1 + "cimi100_base WHERE id='" + Request.QueryString["id"] + "' ");
				if (!rs2.EOF)
				{
				...
				{
					Response.Redirect("cimi100_base.aspx");
				}
			}
		}
		catch
	...
	protected void BUTTON_save_Click(object sender, System.EventArgs e)
	{
		if (this.IsValid)
		{
			...
			try
			{
                ...
                rs.Open("SELECT * FROM " + db1 + "cimi100_base WHERE id='" + Request.QueryString["id"] + "' ");
				if (!rs.EOF)
				{
	...
	{
        this.GoBack("cimi100_base.aspx",new string[]{"cimi100_baseDelete.aspx"});
	}
