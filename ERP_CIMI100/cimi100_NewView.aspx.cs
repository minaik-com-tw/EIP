/*
 * 
 * H14-004  Herzog於 20141020 修正,增加EIP 料號於單筆產生料號時 check ,在送審時再check 一次
...
namespace ERP_CIMI100
{
    public partial class cimi100_NewView : SmoothEnterprise.Web.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            try
            {
                if (!this.IsPostBack)
                {
                    ...
                    if (this.CurrentUser.LogonID.ToString() == "herzog.lin") button_txt.Visible = true; this.Button1.Enabled = true;


                    #endregion

                    if (this.CurrentUser.LogonID.ToString() == "killy.wang" || this.CurrentUser.LogonID.ToString() == "amy.mu")
                    {

                        this.Button2.Visible = true;  //文管可整批產生料號
                    }
        ...
        protected void BUTTON_FlowFeedback_Click(object sender, EventArgs e)
        {
            if (this.FlowFeedback1.ResultType.ToString() == "Next")
            {
                ...
                if (this.CurrentUser.LogonID.ToString() == "killy.wang" || this.CurrentUser.LogonID.ToString() == "amy.mu" || this.CurrentUser.LogonID.ToString() == "elliot.chang")  //需為最後一關文管再check
                {
                    ...
                    // H14-004 增加check 料號是否重複  
                    //   1.ip185 2.cimi100_new
                    DBTransfer gs = new DBTransfer();
                    //string totrows=gs.GetTopfile("SELECT  COUNT(*) tc  FROM [EIPB].[dbo].[cimi100_new] a  "+
                    //    "  inner join (SELECT  b.partno    FROM [EIPB].[dbo].[cimi100_main] a  left join "+
                    //    "  [EIPB].[dbo].[cimi100_new] b  on a.no=b.no  " +
                    //    "   where  a.no  !='" + FIELD_NO.Text + "' and a.status!='V'    ) b   on a.partno=b.partno  Collate Chinese_Taiwan_Stroke_CS_AS " +
                    //    "   where  a.no ='"+FIELD_NO.Text+"'");
                    //if (totrows != "0")
                    //{



                    //    Response.Write("<script language='JavaScript'>window.alert('料號與EIP 資料庫重複！');</script>");
                    //     checkerror = "1";
                    //}
                    //-----------------------------------------------------------
                    ...
                    while (!rs.EOF)
                    {
                        ...
                        //預測料號 分群碼(ima06)  in (C,S,5,9,G,T)

                        if (rs["group_code"].ToString() == "C" || rs["group_code"].ToString() == "S" || rs["group_code"].ToString() == "5" || rs["group_code"].ToString() == "9" || rs["group_code"].ToString() == "G" || rs["group_code"].ToString() == "T")
                        {
                        ...
                        ps.Open("SELECT  * FROM " + db3 + "MINAIK.dbo.IMA_FILE where IMA01='" + rs["partno"].ToString() + "'");
                        if (!ps.EOF)
                        {
                            Response.Write("<script language='JavaScript'>window.alert('料號" + rs["partno"].ToString() + ",與ERP有相同料號！');</script>");
                            checkerror = "1";
        ...
        protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)  //copy item
        {
            ...
            {
                ScriptManager1.AsyncPostBackTimeout = 120;
                int index = Convert.ToInt32(e.CommandArgument);
                GridViewRow row = GridView1.Rows[index];
                SmoothEnterprise.Database.DataSet rs = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead);
                ...
                if (!rs.EOF)
                {
                    ...
                    while (!rss.EOF)
                    {
                        ...
                        rss.MoveNext();
                        ScriptManager.RegisterClientScriptBlock(UpdatePanel1, typeof(UpdatePanel), "aa1", "$(document).unmask();", true); //遮罩效果(關)
                    ...
                    //this.FIELD_partno.Text = rs["partno"].ToString();
        ...
        private bool FIELD_check()
        {
            ...
            //H14-004 判斷 EIP 是否 double
            SmoothEnterprise.Database.DataSet rs = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead);
            //rs.Open("SELECT  COUNT(*) torows FROM [EIPB].[dbo].[cimi100_main] a left join  EIPB.dbo.cimi100_new b on a.no=b.no where b.partno='" + this.FIELD_partno.Text.Trim() + "'    Collate Chinese_Taiwan_Stroke_CS_AS  and a.no!='" + FIELD_NO.Text + "'  and a.status!='V'    ");
            //if (!rs.EOF)
            //{
            //    if(rs["torows"].ToString()!="0") ScriptManager.RegisterClientScriptBlock(UpdatePanel1, typeof(UpdatePanel), "aa1", " alert('請注意!EIP有相同料號');", true);
            //    //this.FIELD_ERP_identical.Checked = true;
            //}
            //rs.Close();
        ...
        protected void FlowFeedback1_FlowStop(object sender, SmoothEnterprise.Flowwork.UI.WebControl.FlowStopEventArgs e)
        {
            ...
            flowname = "料件新增系統";
            SmoothEnterprise.Database.DataSet rss = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead);
            rss.Open("select no,add_user aid,isnull(b.email,'herzog.lin@minaik.com.tw') aemail,b.name aname,c.id bid, " +
                " isnull(c.email,'herzog.lin@minaik.com.tw') bemail,c.name baname from  " + db1 + "cimi100_main a  left join  " + db2 + "dguser b on a.add_user=b.id " +
            ...
            if (!rss.EOF)
            {
                if (e.ResultType == SmoothEnterprise.Flowwork.Control.ReviewResultType.Complete)  //完成審核
                {
                    try
                    {
                        ...
                        if (!rb.EOF)
                        {
                            ...
                            if (this.FIELD_plantid.Text.Trim() != "MAT")
                            {
                                ...
                                if (this.FIELD_plantid.Text.Trim() == "MAP") message.To.Add("tiffany.lo@mapi.com.tw");
                            }
                            else
                            {
                    ...
                    {
                        Utility.log("料件新增 error -" + FIELD_NO.Text + ex.Message);
                    }
                    ...
                    try
                    {
                        ...
                        rs.Open("SELECT   CASE  WHEN p_plantid ='' THEN p_plantid   ELSE  SUBSTRING(REPLACE(p_plantid,'@', ','), 0, LEN(p_plantid)) END  p_plantid ,  " +
                                "partno,replace(replace(replace(component,'\"','~'),'＂','~'),'''','~') component,replace(replace(replace(dwg,'\"','~'),'＂','~'),'''','~') dwg , group_code,Source_code, " +
                        ...
                        while (!rs.EOF)
                        {
                            if (rs["ERP_identical"].ToString().Trim() != "Y")
                            {
                                ...
                                {
                                    string group_code = rs["group_code"].ToString();
                                    string source_code = rs["Source_code"].ToString();
                                    ...
                                    sw.WriteLine("\"" + bb.ToString() + "\",\"" + rs["partno"].ToString() + "\",\"" + rs["Component"].ToString() + "\",\"" + rs["dwg"].ToString() + "\",\"" +
                                                group_code.ToString() + "\",\"" + source_code.ToString() + "\",\"" + rs["Technology_code"].ToString() + "\",\"" + rs["Homemade_code"].ToString() + "\",\"" +
                                ...
                                {
                                    string source_code = rs["Source_code"].ToString();
                                    if (rs["partno"].ToString().Substring(0, 1) == "C" || rs["partno"].ToString().Substring(0, 1) == "S") source_code = "P";  //20130703  MAT C or S料號開頭 皆為採購料件
                                    sw.WriteLine("\"MINAIK\",\"" + rs["partno"].ToString() + "\",\"" + rs["Component"].ToString() + "\",\"" + rs["dwg"].ToString() + "\",\"" +
                                                rs["partno"].ToString().Substring(0, 1) + "\",\"" + source_code.ToString() + "\",\"" + rs["Technology_code"].ToString() + "\",\"" + rs["Homemade_code"].ToString() + "\",\"" +
                                                rs["IQC"].ToString() + "\",\"" + rs["Unit"].ToString() + "\",\"" + rs["cate2"].ToString() + "\",\"" + rs["f_partno"].ToString() + "\",\"" + rs["cate1"].ToString() + "\",\"" +
                    ...
                    catch (Exception ex)
                    {
                        ...
                        File.Copy(@"D:\\CIMI100_XML\\4GL\\" + this.FIELD_plantid.Text.ToString().Trim() + "_A-CIMI100_" + this.FIELD_NO.Text + ".txt", @"D:\\CIMI100_XML\\Error\\" + this.FIELD_plantid.Text.ToString().Trim() + "_A - CIMI100_" + this.FIELD_NO.Text + "-" + DateTime.Now.Millisecond.ToString() + ".txt");
                    }
                    ...
                    //傳檔案到ERP2
                    DBTransfer tyh = new DBTransfer();
                    string detcount = tyh.GetTopfile("SELECT COUNT(*)  FROM [EIPB].[dbo].[cimi100_new] " +
                        " where no='" + FIELD_NO.Text + "'  and (tc_imi02 is not null or  tc_imi021 is not null )");
                    if (detcount != "0") uploadERP2();
                ...
                if (e.ResultType == SmoothEnterprise.Flowwork.Control.ReviewResultType.Terminate || e.ResultType == SmoothEnterprise.Flowwork.Control.ReviewResultType.Return)         //退回
                {
                    ...
                    ShrFlowMail Backsend = new ShrFlowMail();
                    Backsend.RejectResult("5a3f8c87-e0dc-4bbb-8116-661521bfc6c7", flowname.ToString(), this.FIELD_NO.Text, rss["aemail"].ToString(), "--廠區:" + this.FIELD_plantid.Text + "<BR>--申請人員:" + this.FIELD_add_user.Text.Trim(), rss["aname"].ToString(), "http://eip.minaik.com.tw/erp_cimi100/cimi100_newEdit.aspx?id=" + Request.QueryString["id"], "退回單據");
        ...
        protected void Button1_Click1(object sender, EventArgs e)  //單筆產生料號
        {
            this.FIELD_partno.Text = Coding_Principles(this.FIELD_NO.Text, this.rember_item.Value);
        }

        protected void Button2_Click(object sender, EventArgs e)  //整批產生料號
        {
            //先清空料號
            SmoothEnterprise.Database.DataSet rs = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenUpdate);
            //rs.Open("SELECT * FROM " + db1 + "cimi100_new WHERE no='" + this.FIELD_NO.Text + "' ");
            //while (!rs.EOF)
            //{
            //    rs["partno"] = "";
            //    rs.Update();
            ...
            //}
            //rs.Close();
            DBTransfer ns = new DBTransfer();
            ns.RunIUSql("update " + db1 + "cimi100_new  set partno=''  WHERE no='" + this.FIELD_NO.Text + "' ");

            //if (this.FIELD_NO.Text != "Q301-13100139")
            //{
            //update 料號
            SmoothEnterprise.Database.DataSet rss = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead);
            rss.Open("select * from " + db1 + "cimi100_new where no='" + this.FIELD_NO.Text + "' order by  CONVERT(int,item)   ");
            while (!rss.EOF)
            {

                //rs.Open("SELECT * FROM " + db1 + "cimi100_new WHERE no='" + this.FIELD_NO.Text + "' and  item='" + rss["item"].ToString() + "' ");
                //if (!rs.EOF)
                //{
                //    rs["partno"] = Coding_Principles(this.FIELD_NO.Text, rss["item"].ToString());
                //    rs.Update();
                //}
                //rs.Close();
                string partno = Coding_Principles(this.FIELD_NO.Text, rss["item"].ToString());
                ns.RunIUSql("update " + db1 + "cimi100_new  set partno='" + partno + "'  WHERE no='" + this.FIELD_NO.Text + "' and  item='" + rss["item"].ToString() + "'");
        ...
        private string Coding_number(string a, int b, int c, string d) //至ERP尋找料號,找出MAX的自動編碼並+1  a-->前端料號 b-->從第幾碼取  c-->取幾位 d-->這張單的第幾個項次
        {
            //假如有GIT 那第一碼和第二碼要change
            DBTransfer hs = new DBTransfer();
            string plantGIT = hs.GetTopfile("select count(*)  from eipb.dbo.cimi100_new " +
                " where no='" + FIELD_NO.Text + "'  and p_plantid like '%GIT%'");
            if (plantGIT == "1")
            {
                a = a.Substring(1, 1) + a.Substring(0, 1) + a.Substring(2, (a.Length - 2));
            }
            ...
            rs.Open("select max(SubString(IMA01," + b + "," + c + ")) IMAKEY  from " +
                    ...
                    "and partno <>'' and partno like '" + a.ToString() + "%' and item <> '" + d.ToString() + "'  ) a ");
            TextBox1.Text = "select max(SubString(IMA01," + b + "," + c + ")) IMAKEY  from " +
                    "(SELECT  ima01 COLLATE Chinese_Taiwan_Stroke_CS_AS as ima01  FROM " + db3 + "MINAIK.dbo.IMA_FILE WHERE IMA01 LIKE '" + a.ToString() + "%' " +
                    "union    " +
                    "select partno ima01 from  " + db1 + "cimi100_main a left join " + db1 + "cimi100_new b on a.no=b.no where class_no='1' and status <>'V' and partno is not null  " +
                    "and partno <>'' and partno like '" + a.ToString() + "%' and item <> '" + d.ToString() + "'  ) a ";
            ...
            else
            {
                //最大值加1
                //DBTransfer hs = new DBTransfer();
                partno_like = hs.GetTopfile("select target=" + db1 + "[MaterSn] ('" + partno_like + "')");
            }

            return partno_like;
        }

        private string Coding_Principles(string no, string item)  //各編碼原則編碼function
        {
            string partno = "";
            string inside_group = ""; //廠內group
            SmoothEnterprise.Database.DataSet rss = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead);
            rss.Open("select * from " + db1 + "cimi100_new where no='" + no.ToString() + "' and item='" + item.ToString() + "'   ");
            if (!rss.EOF)
            {
                //switch (this.FIELD_Coding_Class.Text)
                switch (rss["coding_class"].ToString().Trim())
                {
                    ...
                    case "1":
                        ...
                        if (rss["a8"].ToString().Length == 0) partno = partno.ToString() + Coding_number(partno, 14, 2, item.ToString()); else partno = partno + rss["a8"].ToString();
                    ...
                    case "2":
                        ...
                        partno = partno.ToString() + Coding_number(partno, 7, 4, item.ToString());

                        break;

                    case "3":
                        partno = rss["c1"].ToString() + rss["c2"].ToString();
                        partno = partno.ToString() + Coding_number(partno, 4, 3, item.ToString());
                    ...
                    case "5":
                        ...
                        partno = partno.ToString() + Coding_number(partno, 7, 3, item.ToString());
                        partno = partno + rss["e5"];
                        break;

                    case "6":
                        partno = rss["f1"].ToString() + rss["f2"].ToString() + rss["f3"].ToString();
                        if (rss["f4"].ToString().Length == 0) partno = partno + Coding_number(partno, 7, 2, item.ToString()); else partno = partno + rss["f4"].ToString();
                    ...
                    case "9":
                        ...
                        partno = partno + Coding_number(partno, 10, 3, item.ToString());
                        break;
                    ...
                    case "11":
                        ...
                        partno = partno + Coding_number(partno, 8, 4, item.ToString()) + rss["k7"].ToString();
                        break;
                    ...
                    case "17":
                        ...
                        partno = partno + Coding_number(partno, 12, 4, item.ToString());
                        break;

                    case "18":
                        partno = rss["R1"].ToString() + rss["R2"].ToString() + rss["R3"].ToString();
                        if (rss["R4"].ToString().Length == 0) partno = partno + Coding_number(partno, 7, 2, item.ToString()); else partno = partno + rss["R4"].ToString();
                    ...
                    case "20":
                        ...
                        if (rss["T4"].ToString().Length == 0) partno = partno + Coding_number(partno, 7, 2, item.ToString()); else partno = partno + rss["T4"].ToString();
                    ...
                    case "21":
                        ...
                        partno = partno + Coding_number(partno, 8, 3, item.ToString());

                        break;

                    case "22":
                        if (rss["v2"].ToString() == "NA") inside_group = rss["v1"].ToString(); else inside_group = rss["v2"].ToString().Substring(0, 1);
                        partno = inside_group.ToString() + rss["V1"].ToString() + rss["V4"].ToString() + rss["V5"].ToString();
                        break;
                    ...
                    case "28":
                        ...
                        partno = partno + Coding_number(partno, 14, 3, item.ToString());

                        break;

                    case "29":
                        partno = rss["AC1"].ToString() + rss["AC2"].ToString() + rss["AC3"].ToString() + rss["AC4"].ToString();
                        partno = partno + Coding_number(partno, 7, 4, item.ToString());
                        partno = partno + rss["AC6"].ToString() + rss["AC7"].ToString();
                        break;

                    case "30":
                        if (rss["AD2"].ToString() == "NA") inside_group = rss["AD1"].ToString(); else inside_group = rss["AD2"].ToString().Substring(0, 1);
                        partno = inside_group.ToString() + rss["AD1"].ToString() + rss["AD3"].ToString() + rss["AD4"].ToString() + rss["AD5"].ToString() + rss["AD6"].ToString() + rss["AD7"].ToString() + rss["AD8"].ToString();
                        break;
                }
            ...
            if (plantGIT == "1")
            {
                partno = partno.Substring(1, 1) + partno.Substring(0, 1) + partno.Substring(2, (partno.Length - 2));
            }
        ...
        //針對一些異常符號 \ ＂ '' & EIP 都會轉成~ ,再由ERP Team 來接手轉換 "
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            rs.Open("SELECT   CASE  WHEN p_plantid ='' THEN p_plantid   ELSE  SUBSTRING(REPLACE(p_plantid,'@', ','), 0, LEN(p_plantid)) END  p_plantid ,  " +
                           ...
                           " replace(replace(replace(replace(dwg,'\"','~'),'＂','~'),'''','~'),'&','~') dwg , group_code,Source_code, " +
                           "CASE WHEN group_code in ('T','G','K','H') THEN partno ELSE null end Technology_code, " +
                           "Homemade_code,IQC,Unit, " +
                           "cate2,f_partno,cate1,cate3,Idle,Customers_class,hsf,'Y' stateus,ERP_identical  " +
                           "FROM " + db1 + " cimi100_new where no='" + this.FIELD_NO.Text.ToString().Trim() + "' order by item");
        ...
        //因為子廠反應在ERP找不到廠商代號 20190603
        private void toCIMI012()
        {
            ...
            if (i > 0)
            { Upload("D:\\CIMI100_XML\\4GL\\" + this.FIELD_plantid.Text.ToString().Trim() + "_-CIMI012_" + this.FIELD_NO.Text + ".txt", "ftp://192.168.0.250/" + this.FIELD_plantid.Text.ToString().Trim() + "_-CIMI012_" + this.FIELD_NO.Text + ".txt", "4gl", "4gl"); }
            else
            {
                File.Delete(fs.Name);
            }
