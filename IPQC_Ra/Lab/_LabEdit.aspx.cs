    ...
    public partial class LabEdit : Lab_Base
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                _leader = "2CA1C1C5-D0C2-405C-91D0-F45DDF9AB462"; //"02CA2090-C5D0-455B-A671-5D7168D08559";
            ...
            else
            {
                ...
                _leader = txt_leader_id.Value;


            }

            txt_leader.Text = Utility.GetUName(_leader);
            btn_upload.Text = getStr("upload_file");

        }

        private void getReport(string lab_id)
        {
            if (!string.IsNullOrEmpty(lab_id))
            {
                ...
                {
                    string sql = string.Format("select * from eipe.dbo.ipqc_ra where rowid='{0}' ", lab_id);

                    rs.Open(sql);

                    if (!rs.EOF)
                    {
                        lbtn_download.Text = rs["report_name"].ToString();
                        lbtn_download.Visible = true;
        ...
        protected void gv_Lab_RowDataBound(object sender, System.Web.UI.WebControls.GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (Index > 0)
                {
                    ...
                    e.Row.Cells[3].Text = Convert.ToDateTime(drv["complate_date"].ToString()).ToString("yyyy/MM/dd");
        ...
        {
            bool ishwo = true;
            if (base._status != status.Waite && base._status != status.Stop)
            {
                ishwo = false;
            }
        ...
        protected void lbtn_download_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(ID))
            {
                using (SmoothEnterprise.Database.DataSet rs = new SmoothEnterprise.Database.DataSet(SmoothEnterprise.Database.DataSetType.OpenRead))
                {
                    rs.Open("select * from eipe.dbo.ipqc_ra_lab where rowid='" + ID + "' ");

                    if (!rs.EOF)
                    {
                        try
                        {
                            base.Response.AddHeader("content-disposition", "attachment;filename=" + rs["report_name"]);
                        ...
                        //透過Header設定檔名 
                        Response.BinaryWrite((byte[])rs["report"]);
        ...
        protected void lbtn_del_Click(object sender, EventArgs e)
        {
            ...
            string files= btn.CommandArgument;
            ...
            if (!string.IsNullOrEmpty(_lab_id))
            {
                ...
                {
                    string sql = string.Format("select * from eipe.dbo.ipqc_ra_lab where id='{0}' ", _lab_id);
                    rs.Open(sql);
                    if (!rs.EOF)
                    { 
                        rs["Report"] = null;
